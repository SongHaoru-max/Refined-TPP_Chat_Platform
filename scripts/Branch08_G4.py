...
raw_rm_pair = input(
    "Enter reporter channel pair for Rm (numerator/denominator, default=129/127): "
).strip()
if not raw_rm_pair:
    raw_rm_pair = "129/127"

try:
    rm_num, rm_den = [x.strip() for x in raw_rm_pair.split("/")]
except Exception:
    raise ValueError("Invalid Rm pair format. Use 'numerator/denominator', e.g. 129/127")

valid_reporters = {"126", "127", "128", "129", "130", "131"}
if rm_num not in valid_reporters or rm_den not in valid_reporters:
    raise ValueError(f"Invalid reporter pair: {raw_rm_pair}")

control_num_col = f"{control_sample} TMT-{rm_num}"
control_den_col = f"{control_sample} TMT-{rm_den}"
missing_ctrl_cols = [c for c in [control_num_col, control_den_col] if c not in df.columns]
if missing_ctrl_cols:
    raise ValueError(f"Control reporter columns not found: {missing_ctrl_cols}")

# In no-technical-replicate mode, only the selected reporter pair must be quantified.
df_control_complete = df.loc[
    (df[control_num_col] != 0) &
    (df[control_den_col] != 0) &
    unique_ge2
].copy()

df_control_complete["Rm_unmod_raw"] = df_control_complete[control_num_col] / df_control_complete[control_den_col]
df_control_complete["Norm_factor"] = 1.0
df_control_complete["Rm_unmod_adjusted"] = df_control_complete["Rm_unmod_raw"] * df_control_complete["Norm_factor"]

cf_df = pd.DataFrame({
    "Metric": ["Rm_unmod_adjusted"],
    "Numerator_Channel": [rm_num],
    "Denominator_Channel": [rm_den],
    "Median_Rm_raw": [df_control_complete["Rm_unmod_raw"].median()],
...
# User input for the sample group to compute Delta Rm
chosen_group = input("Enter the sample group to calculate ΔRm (exact name, e.g., 'N-Glyco'): ").strip()

ch_num_site = f"Intensity {chosen_group} TMT6-{rm_num}"
ch_den_site = f"Intensity {chosen_group} TMT6-{rm_den}"
missing_site_cols = [c for c in [ch_num_site, ch_den_site] if c not in site_df.columns]
if missing_site_cols:
    raise ValueError(f"Columns not found for selected group '{chosen_group}': {missing_site_cols}")

print(f"Selected sample group for ΔRm calculation: '{chosen_group}'")
print(f"Using reporter pair for Rm: {rm_num}/{rm_den}")

print("\n=== Filtering sites with complete quantification in selected-group reporter pair ===")
site_df_clean = site_df.replace(0, np.nan)
complete_mask = site_df_clean[[ch_num_site, ch_den_site]].notna().all(axis=1)
...
print(f"Total quantified sites (complete in selected-group reporter pair): {num_sites}")
...
# ==============================
# Step 5: Calculate Rm and adjusted ΔRm for intersected proteins/sites
# ==============================
...
# Calculate raw/adjusted Rm for each site in selected group.
site_df_complete['Rm_glyco_raw'] = site_df_complete[ch_num_site] / site_df_complete[ch_den_site]
site_df_complete['Norm_factor'] = 1.0
site_df_complete['Rm_glyco_adjusted'] = site_df_complete['Rm_glyco_raw'] * site_df_complete['Norm_factor']

complete_outfile = os.path.join(work_dir, f"N_glyco_sites_Rm_adjusted_{chosen_group}_complete.csv")
site_df_complete.to_csv(complete_outfile, index=False)
print(f"Complete site table with adjusted Rm saved: {complete_outfile}")

# ---------------- Filtered sites ----------------
site_df_filtered = site_df_filtered.copy()
site_df_filtered['Rm_glyco_raw'] = site_df_filtered[ch_num_site] / site_df_filtered[ch_den_site]
site_df_filtered['Norm_factor'] = 1.0
site_df_filtered['Rm_glyco_adjusted'] = site_df_filtered['Rm_glyco_raw'] * site_df_filtered['Norm_factor']

df_control_filtered = df_control_filtered.copy()
df_control_complete = df_control_complete.copy()

# Save filtered protein-level CSV
new_protein_cols = ['Rm_unmod_raw', 'Norm_factor', 'Rm_unmod_adjusted']
...
new_protein_cols_complete = ['Rm_unmod_raw', 'Norm_factor', 'Rm_unmod_adjusted']
...
# ---------------- Site-level ΔRm ----------------
# Normalization factor is fixed to 1.0 in single-run mode.
prot_rm_map = df_control_filtered.set_index('UniProtID')['Rm_unmod_adjusted'].to_dict()
site_df_filtered['Rm_unmod_adjusted'] = site_df_filtered['Protein Accession'].map(prot_rm_map)
site_df_filtered['ΔRm'] = site_df_filtered['Rm_glyco_adjusted'] - site_df_filtered['Rm_unmod_adjusted']

# Save site-level CSV
new_site_cols = ['Rm_glyco_raw', 'Norm_factor', 'Rm_glyco_adjusted', 'Rm_unmod_adjusted', 'ΔRm']
...
site_outfile = os.path.join(work_dir, "N_glyco_sites_with_DeltaRm_adjusted_singleRun.csv")
site_df_filtered[site_cols_to_save].to_csv(site_outfile, index=False)
print(f"Site-level ΔRm table saved: {site_outfile}")

site_df_analysis = site_df_filtered.dropna(subset=['ΔRm']).copy()
analysis_output = os.path.join(work_dir, "N_glyco_sites_analysis_input_noCV.csv")
site_df_analysis.to_csv(analysis_output, index=False)
print(f"No-CV analysis input table saved: {analysis_output}")
print(f"Sites entering normality/significance analysis: {site_df_analysis.shape[0]}")

# -----------------------------
# Step 9: Visualize Delta_Rm distribution and check normality
# -----------------------------
if "ΔRm" not in site_df_analysis.columns:
    raise KeyError("Required ΔRm column missing from analysis table")

delta_rm_for_plot = site_df_analysis["ΔRm"].dropna().astype(float)

plt.figure(figsize=(6,4))
sns.histplot(delta_rm_for_plot, kde=True, color='#A7AED2', bins=30)
plt.title('ΔRm Distribution (N-glycosites entering analysis)')
plt.xlabel('ΔRm')
plt.ylabel('Count')
plt.tight_layout()
...
if delta_rm_not_normal:
    ...
    print("ΔRm is approximately normal: use windowed robust-sigma right-tailed test")

# --- 6. Debug check: confirm original table unchanged ---
print("Step 9 check: site_df_analysis rows =", site_df_analysis.shape[0])

# ------------------------------
# Step 10-11: Significant site identification (branch by normality)
# ------------------------------

# Sort sites by Delta Rm for stable output ordering.
site_df_sorted = site_df_analysis.sort_values("ΔRm").reset_index(drop=True)

if not delta_rm_not_normal:
    print("Step10 strategy: ΔRm normal -> use windowed robust sigma one-tailed test")
    ...
    site_df_final["Analysis_method"] = "normal_window_robust_sigma_right_tailed"
